...
census_sum <- census_sexes_summary_melt_treatment %>% 
                group_by(Date_of_1st_session, Treatment, Sex) %>% 
...
census_sum_plot <- ggplot(census_sexes_summary_melt_treatment, aes(y = value, x = Date_of_1st_session, color = Sex))+ 
    theme_bw() +
...
str(census_sexes_summary_melt_treatment)


# how to test whether the more common sex has a higher decline?
library(lme4)

mod1 <- glm(value ~ Sex * Date_of_1st_session, family = "poisson",
             data = census_sexes_summary_melt_treatment[census_sexes_summary_melt_treatment$Treatment == 0.25, ])
summary(mod1)

mod2 <- glm(value ~ Sex * Date_of_1st_session, family = "poisson",
    data = census_sexes_summary_melt_treatment[census_sexes_summary_melt_treatment$Treatment == 0.5, ])
summary(mod2)

mod3 <- glm(value ~ Sex * Date_of_1st_session, family = "poisson",
    data = census_sexes_summary_melt_treatment[census_sexes_summary_melt_treatment$Treatment == 0.75, ])
summary(mod3)
#geom_line(data = census_sum, aes(y = nind, x = Date_of_1st_session, color = Sex))


# 

# idea here
# weigh means of pops by num of individuals 

# calculate weighted mean ASR by number of individuals in the population ---------------------------
...
# plot the ASR temporal dynamics of each Treatment -------------------------------------------------
time_ASR_census_plot <- 
    census_sexes_summary_melt_treatment %>% 
    mutate(Treatment = as.factor(Treatment)) %>% 
    group_by(Treatment, Date_of_1st_session, Sex) %>% 
    summarise(ASR = mean(ASR)) %>% 
...
weights_df <- gather(census_sexes_weighted_mean, Femal)
...
ggsave(weighted_mean_plot,
    filename = "weighted_asr_mean.jpg",
    path = "figs/",
    width = 8,
    height = 3, units = "in",
    dpi = 300)
...
# models? what about zero?
# histogram of female matings?
# 
